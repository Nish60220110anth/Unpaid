...
import HeaderA from "../rows/HeaderA";
import HeaderB from "../rows/HeaderB";
	...
	exportCSV() {
		...
		for (var r = beginRowCount; r < endRowCount; r++) {
			...
			for (var i = 0; i < 18; i++) {
				...
				emptyStringifyArray[2] = Math.round(
					(this.state["category" + (r - 4)][1] / this.columnSum("C")) * 100
				).toString();
			}
	...
	render() {
		return (
			<div className="form-vertical">
				...
				<table>
					<tbody>
						...
						<InputRow
							...
							categoryNumberProp={1}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category2[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={2}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category3[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={3}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category4[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={4}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category5[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={5}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category6[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={6}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category7[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={7}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category8[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={8}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category9[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={9}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category10[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={10}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category11[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={11}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<InputRow
							dProp={Math.round(
								(this.state.category12[1] / this.columnSum("C")) * 100
							)}
							categoryNumberProp={12}
							fields={inputFieldIDs}
							updateGrandpaState={this.updateGrandpaData}
						></InputRow>
						<Footer
							cProp={this.columnSum("C")}
				...
				<Row>
					<div>
						...
						<ComposedChart
							...
							data={[
								{
									name: this.state.category1[0],
									Budget: this.state.category1[1],
									Spent: this.state.category1[3],
								},
								{
									name: this.state.category2[0],
									Budget: this.state.category2[1],
									Spent: this.state.category2[3],
								},
								{
									name: this.state.category3[0],
									Budget: this.state.category3[1],
									Spent: this.state.category3[3],
								},
								{
									name: this.state.category4[0],
									Budget: this.state.category4[1],
									Spent: this.state.category4[3],
								},
								{
									name: this.state.category5[0],
									Budget: this.state.category5[1],
									Spent: this.state.category5[3],
								},
								{
									name: this.state.category6[0],
									Budget: this.state.category6[1],
									Spent: this.state.category6[3],
								},
								{
									name: this.state.category7[0],
									Budget: this.state.category7[1],
									Spent: this.state.category7[3],
								},
								{
									name: this.state.category8[0],
									Budget: this.state.category8[1],
									Spent: this.state.category8[3],
								},
								{
									name: this.state.category9[0],
									Budget: this.state.category9[1],
									Spent: this.state.category9[3],
								},
								{
									name: this.state.category10[0],
									Budget: this.state.category10[1],
									Spent: this.state.category10[3],
								},
								{
									name: this.state.category11[0],
									Budget: this.state.category11[1],
									Spent: this.state.category11[3],
								},
								{
									name: this.state.category12[0],
									Budget: this.state.category12[1],
									Spent: this.state.category12[3],
								},
							]}
					...
					<div>
						...
						<BarChart
							...
							data={[
								{ month: "JAN", Monthly: this.columnSum("H") },
						...
						<Row>
							<PieChart width={500} height={300}>
								<Pie
									data={[
										{
											category: this.state.category1[0],
											Percentage: Math.round(
												(this.state.category1[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category2[0],
											Percentage: Math.round(
												(this.state.category2[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category3[0],
											Percentage: Math.round(
												(this.state.category3[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category4[0],
											Percentage: Math.round(
												(this.state.category4[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category5[0],
											Percentage: Math.round(
												(this.state.category5[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category6[0],
											Percentage: Math.round(
												(this.state.category6[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category7[0],
											Percentage: Math.round(
												(this.state.category7[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category8[0],
											Percentage: Math.round(
												(this.state.category8[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category9[0],
											Percentage: Math.round(
												(this.state.category9[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category10[0],
											Percentage: Math.round(
												(this.state.category10[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category11[0],
											Percentage: Math.round(
												(this.state.category11[1] / this.columnSum("C")) * 100
											),
										},
										{
											category: this.state.category12[0],
											Percentage: Math.round(
												(this.state.category12[1] / this.columnSum("C")) * 100
											),
										},
				...
				<Button id="csvButton" onClick={() => this.exportCSV()}>
